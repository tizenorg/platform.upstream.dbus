...
{
  DBusMessageIter entry_iter, var_iter, sub_array_iter;
  ...
  if (!dbus_message_iter_open_container (&var_iter,
                                         DBUS_TYPE_ARRAY,
  ...
  if (!dbus_message_iter_append_fixed_array (&sub_array_iter, DBUS_TYPE_BYTE,
                                             &value, value_len))
    {
      dbus_message_iter_abandon_container (&var_iter, &sub_array_iter);
      _dbus_asv_abandon_entry (arr_iter, &entry_iter, &var_iter);
      return FALSE;
    }

  if (!dbus_message_iter_close_container (&var_iter, &sub_array_iter))
    {
      _dbus_asv_abandon_entry (arr_iter, &entry_iter, &var_iter);
      return FALSE;
    }
